...
def create_bar_chart(hoverData, data):
    if hoverData:
        ...
        df_to_plot = df[df['full_fips'] == FIPS].groupby('category_name', as_index=False).sum()[['category_name', 'sale_dollars', 'volume_sold_liters']].sort_values('sale_dollars', ascending=False).head(6)
...
def update_top_store(hoverData, data):
    if hoverData:
        df = pd.DataFrame(data)
        FIPS = hoverData['points'][0]['location']
        temp_ = df[df['full_fips'] == FIPS].groupby(['store_number', 'store_name'], as_index=False).sum()[['store_number', 'store_name', 'sale_dollars']].sort_values('sale_dollars', ascending=False).head(1).reset_index()
        store_name = temp_['store_name'][0].split(' ')[0].capitalize()
        return html.H4(f'💸  #1 buyer:    {store_name} -- USD {int(temp_["sale_dollars"][0])}', style = {'text-align':'left', 'color': '#ba1e7f', 'margin-left': '0.3rem', 'margin-right': '0.2rem', 'margin-top': '5px'})
...
def update_top_store(hoverData, data):
    if hoverData:
        df = pd.DataFrame(data)
        FIPS = hoverData['points'][0]['location']
        temp_ = df[df['full_fips'] == FIPS].groupby(['vendor_number', 'vendor_name'], as_index=False).sum()[['vendor_number', 'vendor_name', 'sale_dollars']].sort_values('sale_dollars', ascending=False).head(1).reset_index()
        vendor_name = temp_['vendor_name'][0].split(' ')[0].capitalize()
        return html.H4(f'💸  #1 vendor:   {vendor_name} -- USD {int(temp_["sale_dollars"][0])}', style = {'text-align':'left', 'color': '#ba1e7f', 'margin-left': '0.3rem', 'margin-right': '0.2rem', 'margin-top': '5px'})
...
def update_top_store(hoverData, data):
    if hoverData:
        df = pd.DataFrame(data)
        FIPS = hoverData['points'][0]['location']
        temp_ = df[df['full_fips'] == FIPS].groupby(['item_number', 'item_description'], as_index=False).sum()[['item_number', 'item_description', 'sale_dollars']].sort_values('sale_dollars', ascending=False).head(1).reset_index()
